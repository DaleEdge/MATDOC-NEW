      ...
      function fetchData(page) {
         $.ajax({
            ...
            success: function(data) {
               ...
               html += '<thead><tr><th>Quota</th><th>Category</th><th>State</th><th>Institute</th><th>Course</th><th>Fee</th><th>Beds</th><th>All India Rank</th></tr></thead><tbody class="table-body-contents">';
               if (data.data.length) {
               ...
               if (data.last_page > 1) {
                  ...
                  data.links.forEach(function(link, index_num) {
                     if (link.url) {
                        ...
                        paginationHtml += '<a class="page-link page_click" href="/ug/closing-rank-details?page=' + index_num + '" data-page="' + new URL(link.url).searchParams.get('page') + '">' + link.label + '</button>';
                        paginationHtml += '</li>';
                     } else {
               ...
               if ($('#closingRankDetailsModal').hasClass('show') == true) {
                  $(".table-body-contents").empty();
                  $('#closingRankDetailsModalBody').html(html + paginationHtml)
               }
            },
