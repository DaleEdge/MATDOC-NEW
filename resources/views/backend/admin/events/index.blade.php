            ...
            <div class="card card-danger card-tabs">
                <div class="card-header p-0">
                    <ul class="nav nav-tabs" role="tablist">
                        ...
                        <li class="nav-item">
                            <a class="nav-link @if(Route::currentRouteName() == 'upcoming_events.index')active @endif" href="{{route('previous_events.index')}}" role="tab" aria-selected="true">Previous Events</a>
                        </li>
                                    ...
                                    @if(!empty($upcoming_events) && $upcoming_events->count())
                                        ...
                                        @foreach ($upcoming_events as $events)
                                            <tr>
                                                <td>{{$i++}}</td>
                                                <td>
                                                    @if ($events->end_date > time())
                                                        <span class="right badge badge-info">Upcoming Event</span> 
                                                    @endif
                                                    @if ($events->end_date < time())
                                                        <span class="right badge badge-warning">Previous Event</span> 
                                                    @endif
                                                </td>
